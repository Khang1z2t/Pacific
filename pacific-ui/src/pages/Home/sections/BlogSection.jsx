...
export const BlogSection = () => {
    ...
    const success = () => {
        messageApi
            ...
            .then(() => message.success('Test OK!', 2.5))
            .then(() => message.info('OK CAI CON C', 2.5));
    };
    return (
            ...
            <div className="max-w-7xl mx-auto px-4">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
                    {pageItem.map((item, index) => (
                        <BlogCards key={index} title={item.title} img={item.img} date={item.date} />
                    ))}
                </div>
                {isLastPage ? (
                    <div className={"text-center mt-8 gap-4"}>
                        <button onClick={() => {setCurrentPage(1)}} className="bg-orange-500 text-white px-4 py-2 rounded-full hover:bg-orange-600 me-2">
                            <FontAwesomeIcon icon={faArrowLeft} />
                        </button>
                        <button onClick={success} className="bg-orange-500 text-white px-4 py-2 rounded-md hover:bg-orange-600">
                            Xem thêm
                        </button>
                        {contextHolder}
                    </div>
                ) : (
                    <Pagination defaultCurrent={1} total={BlogCardItems.length} align={"center"} pageSize={ITEM_PER_PAGE} onChange={onPageChange} className="mt-8 text-center" />
            )}
            </div>
        </section>
    );
