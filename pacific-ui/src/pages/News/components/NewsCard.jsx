...
import { FaEye, FaHeart, FaClock, FaShareAlt, FaBookmark } from 'react-icons/fa';
...
export const NewsCard = ({ blog }) => {
    ...
    return (
        <div className="bg-white rounded-lg shadow-md overflow-hidden transition-all duration-300 hover:shadow-xl hover:-translate-y-1 mb-6 border border-gray-100">
            <Link to={`${config.routes.news}/${blog.slug}`} className="block">
                <div className="flex flex-col md:flex-row">
                    ...
                    <div className="relative w-full md:w-1/3 h-48 md:h-56 overflow-hidden">
                        <img
                            src={blog.featuredImage || config.webConfig.banner1}
                            alt={blog.title}
                        ...
                        {blog.category && (
                            <div className="absolute bottom-0 left-0 bg-gradient-to-r from-blue-600 to-blue-400 text-white text-xs px-3 py-1 rounded-tr-lg">
                                {blog.category.name}
                            </div>
                        )}
                    </div>
                    ...
                    <div className="w-full md:w-2/3 p-5 flex flex-col justify-between">
                        <div>
                            <h2 className="text-xl md:text-2xl font-bold text-gray-800 line-clamp-2 mb-2 hover:text-blue-600 transition-colors duration-300">
                                {blog.title}
                            </h2>
            ...
            <div className="flex justify-end px-4 py-2 bg-gray-50 border-t border-gray-100">
                <button className="text-gray-500 hover:text-blue-600 p-1 transition-colors duration-200" title="Lưu bài viết">
                    <FaBookmark />
                </button>
                <button className="text-gray-500 hover:text-blue-600 p-1 ml-3 transition-colors duration-200" title="Chia sẻ">
                    <FaShareAlt />
                </button>
            </div>
        </div>
